...
function SettingsModal({ keys, setKeys, onClose, voiceId, setVoiceId, darkMode, setDarkMode, aiTone, setAiTone, userName, setUserName, welcomeMessage, setWelcomeMessage, voiceSpeed, setVoiceSpeed }: any) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#0d1018', border: `1px solid ${C.border2}`, borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' as const }}>
        ...
        <div style={{ marginBottom: 20, padding: '12px 14px', background: '#131720', borderRadius: 10, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setDarkMode(!darkMode)} style={{
            ...
            <div style={{ position: 'absolute' as const, top: 3, left: darkMode ? 25 : 3, width: 20, height: 20, borderRadius: '50%', background: '#0d1018', transition: 'left 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.2)' }} />
          </button>
        </div>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <input type="text" value={userName} onChange={e => setUserName(e.target.value)}
            ...
            style={{ width: '100%', background: '#131720', border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 14px', color: C.text, fontFamily: font, fontSize: 13, outline: 'none', boxSizing: 'border-box' as const }} />
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 10, color: C.textDim }}>🪖Â Drill Sergeant</span>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
          ...
          <button onClick={onClose} style={{ flex: 1, background: '#131720', color: C.textDim, border: `1px solid ${C.border}`, borderRadius: 8, padding: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: font }}>Cancel</button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: darkMode ? '#080a0f' : '#f0f4f8', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: font, transition: 'background 0.3s' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
        ...
        {tab === 'cockpit' && (
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: '#080a0f' }}>
            ...
            <div style={{ flex: 1, padding: 16, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, background: '#050609' }}>
              ...
              {/* Signal Hero */}
              <div style={{ background: '#0d1018', borderRadius: 10, padding: 18, position: 'relative', overflow: 'hidden', boxShadow: '0 2px 20px rgba(0,212,160,0.1)', borderTop: '3px solid #6620d4' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {/* Options Flow mini */}
                <div style={{ background: '#0d1018', borderRadius: 7, padding: 10, boxShadow: '0 2px 10px rgba(0,153,204,0.09)', borderLeft: '3px solid #0099cc' }}>
                ...
                <div style={{ background: '#0d1018', borderRadius: 7, padding: 10, boxShadow: '0 2px 10px rgba(0,212,160,0.09)', borderLeft: '3px solid #6620d4' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              {marketScore && (
                <div style={{ background: '#0d1018', borderRadius: 8, padding: '12px 14px', boxShadow: '0 2px 10px rgba(100,140,220,0.08)', borderLeft: `3px solid ${marketScore.color}` }}>
          ...
          <div style={{ flex: 1, display: 'flex', overflow: 'hidden', background: '#050609' }}>
            ...
            <div style={{ width: 240, background: '#0d1018', borderRight: `1px solid rgba(0,212,160,0.1)`, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 0, flexShrink: 0, boxShadow: '2px 0 8px rgba(100,140,220,0.06)' }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea
                  ...
                  rows={5}
                  style={{ width: '100%', background: '#131720', border: `1px solid rgba(255,255,255,0.1)`, borderRadius: 6, padding: '10px 12px', color: C.text, fontSize: 12, outline: 'none', fontFamily: font, resize: 'vertical' as const, lineHeight: 1.6, boxSizing: 'border-box' as const }}
                />
              </div>
              ...
              {showAddPlaybook && (
                <div style={{ marginTop: 8, background: '#131720', border: '1px solid rgba(100,140,220,0.15)', borderRadius: 8, padding: 10 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0, background: '#050609' }}>
              ...
              {(() => {
                const probs = calcProbabilities({ bias: morningPlan.bias, gapDirection: morningPlan.gapDirection, gapSize: morningPlan.gapSize, impliedMove: morningPlan.impliedMove, vixPrice, tiingoContext })
                return (
                  <div style={{ background: '#0d1018', margin: '14px 14px 0', borderRadius: 10, padding: '14px 16px', boxShadow: '0 2px 12px rgba(100,140,220,0.08)', borderTop: `3px solid ${probs.hasData ? probs.dominantColor : 'rgba(100,140,220,0.2)'}` }}>
              ...
              {/* AI Morning Brief */}
              <div style={{ background: '#0d1018', margin: '14px', borderRadius: 10, boxShadow: '0 2px 12px rgba(0,212,160,0.08)', borderTop: '3px solid #6620d4', overflow: 'hidden' }}>
                {/* Header */}
                  ...
                  {customChecklist.map((item: any, idx: number) => (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                      <input value={item.label} onChange={e => setCustomChecklist((p: any[]) => p.map((c, i) => i === idx ? {...c, label: e.target.value} : c))}
                        style={{ flex: 1, background: '#131720', border: `1px solid ${C.border}`, borderRadius: 5, padding: '5px 8px', color: C.text, fontSize: 11, outline: 'none', fontFamily: font }} />
                      <select value={item.category} onChange={e => setCustomChecklist((p: any[]) => p.map((c, i) => i === idx ? {...c, category: e.target.value} : c))}
                        style={{ background: '#131720', border: `1px solid ${C.border}`, borderRadius: 5, padding: '5px 4px', color: C.textDim, fontSize: 10, outline: 'none', fontFamily: font }}>
        ...
        {tab === 'deepdive' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
              ...
              {/* Left panel */}
              <div style={{ width: 180, background: '#0d1018', borderRight: `1px solid rgba(0,212,160,0.1)`, padding: 10, overflowY: 'auto', flexShrink: 0 }}>
                ...
                {morningPlan.bias ? (
                  <div style={{ background: '#131720', border: `1px solid ${C.border}`, borderRadius: 4, padding: '7px 8px', marginBottom: 6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
            ...
            <div style={{ width: 300, background: '#0d1018', borderRight: `1px solid rgba(0,212,160,0.1)`, padding: 16, overflowY: 'auto', flexShrink: 0 }}>
              ...
              {[
                ...
                <div key={key} style={{ marginBottom: 10 }}>
                  ...
                  <input value={(newTrade as any)[key]} onChange={e => setNewTrade(p => ({ ...p, [key]: e.target.value }))}
                    placeholder={ph} style={{ width: '100%', background: '#131720', border: `1px solid ${C.border2}`, borderRadius: 6, padding: '6px 10px', color: C.text, fontSize: 12, outline: 'none' }} />
                </div>
              ))}
              ...
              <div style={{ marginBottom: 10 }}>
                ...
                <select value={newTrade.playbook} onChange={e => setNewTrade(p => ({ ...p, playbook: e.target.value }))}
                  style={{ width: '100%', background: '#131720', border: `1px solid ${C.border2}`, borderRadius: 6, padding: '6px 10px', color: C.text, fontSize: 12, outline: 'none' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  {tradeStats && (
                    <div style={{ fontSize: 10, color: C.textDim, background: '#131720', border: `1px solid ${C.border}`, borderRadius: 5, padding: '2px 8px' }}>
        ...
        {tab === 'journal' && (()=>{           const allT: any[] = trades           const netPnl: number = allT.reduce((s,t)=>s+(Number(t.pnl)||0),0)           const wins: any[] = allT.filter(t=>Number(t.pnl)>0)           const losses: any[] = allT.filter(t=>Number(t.pnl)<0)           const winPct: number = allT.length?Math.round(wins.length/allT.length*100):0           const avgW: number = wins.length?wins.reduce((s,t)=>s+Number(t.pnl),0)/wins.length:0           const avgL: number = losses.length?Math.abs(losses.reduce((s,t)=>s+Number(t.pnl),0)/losses.length):0           const pf: number = avgL>0?avgW/avgL:0           const bigW: number = wins.length?Math.max(...wins.map(t=>Number(t.pnl))):0           const bigL: number = losses.length?Math.max(...losses.map(t=>Math.abs(Number(t.pnl)))):0           const byDay: Record<string,{pnl:number}> = {}           allT.forEach(t=>{const k=String(t.date||'');if(!byDay[k])byDay[k]={pnl:0};byDay[k].pnl+=Number(t.pnl)||0})           const dayArr=Object.entries(byDay)           const dayW=dayArr.filter(([,v])=>v.pnl>0).length           const dayWpct=dayArr.length?Math.round(dayW/dayArr.length*100):0           const byHr: Record<number,number>={}           allT.forEach(t=>{const hv=t.time?parseInt(t.time):null;if(hv!==null){byHr[hv]=(byHr[hv]||0)+(Number(t.pnl)||0)}})           const hrEntries=Object.entries(byHr).map(([h,p])=>({h:parseInt(h),p:Number(p)}))           const topHr=hrEntries.length?[...hrEntries].sort((a,b)=>b.p-a.p)[0]:null           const botHr=hrEntries.length?[...hrEntries].sort((a,b)=>a.p-b.p)[0]:null           const maxHrAbs=hrEntries.length?Math.max(...hrEntries.map(e=>Math.abs(e.p)),1):1           const byPb: Record<string,{w:number,tot:number,pnl:number}>={}           allT.forEach(t=>{const k=String(t.playbook||'No playbook');if(!byPb[k])byPb[k]={w:0,tot:0,pnl:0};byPb[k].tot++;if(Number(t.pnl)>0)byPb[k].w++;byPb[k].pnl+=Number(t.pnl)||0})           const pbEntries=Object.entries(byPb).sort((a,b)=>b[1].pnl-a[1].pnl)           const now2=new Date(),yr=now2.getFullYear(),mo=now2.getMonth()           const dim=new Date(yr,mo+1,0).getDate(),fdm=new Date(yr,mo,1).getDay(),mos=String(mo+1).padStart(2,'0')           const monthLabel=new Date(yr,mo).toLocaleString('default',{month:'long',year:'numeric'})           const analyzePatterns=async()=>{setAiLoading(true);try{const pbStr=pbEntries.map(([n,pb])=>n+': '+pb.tot+'t '+Math.round(pb.w/pb.tot*100)+'% $'+Math.round(pb.pnl)).join(' | ');const hrStr=hrEntries.sort((a,b)=>a.h-b.h).map(e=>e.h+'=$'+Math.round(e.p)).join(', ');const prompt='Analyze SPX trader. Win:'+winPct+'% PnL:$'+Math.round(netPnl)+' AvgW:$'+Math.round(avgW)+' AvgL:$'+Math.round(avgL)+' PF:'+pf.toFixed(2)+'x DayWin:'+dayWpct+'% PB:'+pbStr+' HR:'+hrStr;const resp=await fetch('/api/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:600,messages:[{role:'user',content:prompt}]})});const dat=await resp.json();const analysis=dat.content?.[0]?.text||'No analysis';setChatMessages([{role:'assistant',content:'Pattern Analysis: '+analysis}]);setTab('cockpit')}catch(e){console.error(e)};setAiLoading(false)}           return(<div style={{flex:1,overflowY:'auto',padding:20,background:'#050609'}}><div style={{fontFamily:fontDisplay,fontSize:16,fontWeight:700,color:C.text,marginBottom:16}}>Performance Analytics</div>{allT.length===0?(<div style={{textAlign:'center',padding:'60px 20px',background:'#0d1018',borderRadius:12,border:'1px solid '+C.border}}><div style={{fontSize:40,marginBottom:12}}>📊</div><div style={{fontSize:14,color:C.textDim,marginBottom:8}}>No trade data yet</div><div style={{fontSize:12,color:C.textMuted}}>Import a CSV in LOG TRADE to unlock analytics</div></div>):(<div><div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,marginBottom:16}}><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Net P&L</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:netPnl>=0?C.synapse:C.red}}>{netPnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(netPnl).toLocaleString()}</div><div style={{fontSize:10,color:C.textMuted}}>{allT.length} trades</div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Trade Win %</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:winPct>=60?C.synapse:winPct>=50?C.yellow:C.red}}>{winPct}%</div><div style={{fontSize:10,color:C.textMuted}}>{wins.length}/{allT.length}</div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Win</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.synapse}}>+{String.fromCharCode(36)}{Math.round(avgW)}</div><div style={{fontSize:10,color:C.textMuted}}>Per winner</div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Loss</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.red}}>-{String.fromCharCode(36)}{Math.round(avgL)}</div><div style={{fontSize:10,color:C.textMuted}}>Per loser</div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Day Win %</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:dayWpct>=60?C.synapse:dayWpct>=50?C.yellow:C.red}}>{dayWpct}%</div><div style={{fontSize:10,color:C.textMuted}}>{dayW}/{dayArr.length} days</div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Profit Factor</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:pf>=1.5?C.synapse:pf>=1?C.yellow:C.red}}>{pf.toFixed(2)}x</div><div style={{fontSize:10,color:C.textMuted}}>Win/loss ratio</div></div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Daily P&L — {monthLabel}</div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:4}}>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=><div key={d} style={{textAlign:'center',fontSize:10,color:C.textMuted}}>{d}</div>)}</div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>{Array.from({length:fdm}).map((_,i)=><div key={'e'+i} style={{aspectRatio:'1'}}/>)}{Array.from({length:dim}).map((_,i)=>{const day=i+1,ds=yr+'-'+mos+'-'+String(day).padStart(2,'0'),dd=byDay[ds],dow=(fdm+i)%7,isWe=dow===0||dow===6;return <div key={day} style={{aspectRatio:'1',borderRadius:5,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:isWe?'transparent':dd?(dd.pnl>=0?'rgba(0,170,85,0.15)':'rgba(204,16,64,0.12)'):'#131720',border:'1px solid '+(dd?(dd.pnl>=0?'rgba(0,170,85,0.3)':'rgba(204,16,64,0.25)'):C.border)}}><span style={{fontSize:9,color:C.textMuted}}>{day}</span>{dd&&<span style={{fontSize:9,fontWeight:700,color:dd.pnl>=0?C.synapse:C.red}}>{dd.pnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(dd.pnl)}</span>}</div>})}</div></div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Win / Loss Breakdown</div><div style={{height:8,borderRadius:4,background:'#1a1f2e',overflow:'hidden',marginBottom:8}}><div style={{height:'100%',width:winPct+'%',background:C.synapse,borderRadius:4}}/></div><div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:14}}><span style={{color:C.synapse,fontWeight:700}}>{winPct}% wins</span><span style={{color:C.red,fontWeight:700}}>{100-winPct}% losses</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Total</span><span style={{fontWeight:700,color:C.text}}>{allT.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Winners</span><span style={{fontWeight:700,color:C.synapse}}>{wins.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Losers</span><span style={{fontWeight:700,color:C.red}}>{losses.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Largest win</span><span style={{fontWeight:700,color:C.synapse}}>+{String.fromCharCode(36)}{Math.round(bigW)}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Largest loss</span><span style={{fontWeight:700,color:C.red}}>-{String.fromCharCode(36)}{Math.round(bigL)}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',fontSize:12}}><span style={{color:C.textDim}}>Profit factor</span><span style={{fontWeight:700,color:pf>=1?C.synapse:C.red}}>{pf.toFixed(2)}x</span></div></div><div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Playbook Performance</div>{pbEntries.length===0?<div style={{fontSize:12,color:C.textMuted}}>Log trades with playbooks to see breakdown</div>:pbEntries.map(([name,pb])=>{const pbWr=pb.tot?Math.round(pb.w/pb.tot*100):0;return <div key={name} style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{fontSize:11,color:C.text,fontWeight:600}}>{name}</span><span style={{fontSize:11,fontWeight:700,color:pb.pnl>=0?C.synapse:C.red}}>{pb.pnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(pb.pnl)}</span></div><div style={{display:'flex',gap:8,fontSize:10,color:C.textMuted,marginBottom:4}}><span>{pb.tot} trades</span><span style={{color:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}>{pbWr}% win</span></div><div style={{height:4,borderRadius:2,background:'#1a1f2e',overflow:'hidden'}}><div style={{height:'100%',width:pbWr+'%',background:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}/></div></div>})}</div></div>{hrEntries.length>0&&(<div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>P&L by Hour of Day</div><div style={{display:'flex',gap:4,alignItems:'flex-end',height:80,marginBottom:8}}>{Array.from({length:14},(_,i)=>i+8).map(h=>{const hp=byHr[h]||0,hpct=Math.abs(hp)/maxHrAbs*100;return <div key={h} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:2}}><div style={{width:'100%',display:'flex',flexDirection:'column',justifyContent:hp>=0?'flex-end':'flex-start',height:60}}><div style={{width:'100%',height:hpct+'%',minHeight:hp!==0?2:0,background:hp>=0?'rgba(0,170,85,0.7)':'rgba(204,16,64,0.7)',borderRadius:2}}/></div><span style={{fontSize:8,color:C.textMuted}}>{h>12?h-12+'p':h+'a'}</span></div>})}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{topHr&&<div style={{background:'rgba(0,170,85,0.08)',border:'1px solid rgba(0,170,85,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Best hour</div><div style={{fontSize:12,color:C.synapse,fontWeight:700}}>{topHr.h>12?topHr.h-12+'PM':topHr.h+'AM'} (+{String.fromCharCode(36)}{Math.round(topHr.p)})</div></div>}{botHr&&<div style={{background:'rgba(204,16,64,0.06)',border:'1px solid rgba(204,16,64,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Worst hour</div><div style={{fontSize:12,color:C.red,fontWeight:700}}>{botHr.h>12?botHr.h-12+'PM':botHr.h+'AM'} (-{String.fromCharCode(36)}{Math.round(Math.abs(botHr.p))})</div></div>}</div></div>)}<div style={{background:'#0d1018',border:'1px solid '+C.tealBorder,borderRadius:10,padding:16}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><div style={{width:8,height:8,borderRadius:'50%',background:C.teal,animation:'pulse 2s infinite'}}/><div style={{fontFamily:fontDisplay,fontSize:12,fontWeight:700,color:C.teal}}>AI Pattern Recognition</div></div><button onClick={analyzePatterns} style={{background:C.tealDim,border:'1px solid '+C.tealBorder,borderRadius:8,padding:'10px 16px',color:C.teal,cursor:'pointer',fontFamily:font,fontSize:12,fontWeight:700,marginBottom:12}}>{aiLoading?'↻ Analyzing...':'🔍 Analyze My Trading Patterns'}</button><div style={{fontSize:11,color:C.textDim,lineHeight:1.6}}>Your trade history, hourly patterns, and playbook data feed the AI companion.</div></div></div>)}</div>)         })()}
          ...
          allT.forEach(t=>{const k=String(t.date||'');if(!byDay[k])byDay[k]={pnl:0};byDay[k].pnl+=Number(t.pnl)||0})
          ...
          const topHr=hrEntries.length?[...hrEntries].sort((a,b)=>b.p-a.p)[0]:null
          const botHr=hrEntries.length?[...hrEntries].sort((a,b)=>a.p-b.p)[0]:null
          ...
          allT.forEach(t=>{const k=String(t.playbook||'No playbook');if(!byPb[k])byPb[k]={w:0,tot:0,pnl:0};byPb[k].tot++;if(Number(t.pnl)>0)byPb[k].w++;byPb[k].pnl+=Number(t.pnl)||0})
          const pbEntries=Object.entries(byPb).sort((a,b)=>b[1].pnl-a[1].pnl)
          ...
          const monthLabel=new Date(yr,mo).toLocaleString('default',{month:'long',year:'numeric'})
          const analyzePatterns=async()=>{
            setAiLoading(true)
            try{
              const pbStr=pbEntries.map(([n,pb])=>n+': '+pb.tot+'t '+Math.round(pb.w/pb.tot*100)+'% $'+Math.round(pb.pnl)).join(' | ')
              const hrStr=hrEntries.sort((a,b)=>a.h-b.h).map(e=>e.h+'=$'+Math.round(e.p)).join(', ')
              const prompt='Analyze SPX trader. Win:'+winPct+'% PnL:$'+Math.round(netPnl)+' AvgW:$'+Math.round(avgW)+' AvgL:$'+Math.round(avgL)+' PF:'+pf.toFixed(2)+'x DayWin:'+dayWpct+'% PB:'+pbStr+' HR:'+hrStr
              const resp=await fetch('/api/ai',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:600,messages:[{role:'user',content:prompt}]})})
              const dat=await resp.json()
              const analysis=dat.content?.[0]?.text||'No analysis'
              setChatMessages([{role:'assistant',content:'Pattern Analysis: '+analysis}])
              setTab('cockpit')
            }catch(e){console.error(e)}
            setAiLoading(false)
          }
          return(<div style={{flex:1,overflowY:'auto',padding:20,background:'#050609'}}>
            <div style={{fontFamily:fontDisplay,fontSize:16,fontWeight:700,color:C.text,marginBottom:16}}>Performance Analytics</div>
            {allT.length===0?(<div style={{textAlign:'center',padding:'60px 20px',background:'#0d1018',borderRadius:12,border:'1px solid '+C.border}}><div style={{fontSize:40,marginBottom:12}}>📊</div><div style={{fontSize:14,color:C.textDim,marginBottom:8}}>No trade data yet</div><div style={{fontSize:12,color:C.textMuted}}>Import a CSV in LOG TRADE to unlock analytics</div></div>):(
            <div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,marginBottom:16}}>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Net P&L</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:netPnl>=0?C.synapse:C.red}}>{netPnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(netPnl).toLocaleString()}</div><div style={{fontSize:10,color:C.textMuted}}>{allT.length} trades</div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Trade Win %</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:winPct>=60?C.synapse:winPct>=50?C.yellow:C.red}}>{winPct}%</div><div style={{fontSize:10,color:C.textMuted}}>{wins.length}/{allT.length}</div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Win</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.synapse}}>+{String.fromCharCode(36)}{Math.round(avgW)}</div><div style={{fontSize:10,color:C.textMuted}}>Per winner</div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Loss</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.red}}>-{String.fromCharCode(36)}{Math.round(avgL)}</div><div style={{fontSize:10,color:C.textMuted}}>Per loser</div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Day Win %</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:dayWpct>=60?C.synapse:dayWpct>=50?C.yellow:C.red}}>{dayWpct}%</div><div style={{fontSize:10,color:C.textMuted}}>{dayW}/{dayArr.length} days</div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Profit Factor</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:pf>=1.5?C.synapse:pf>=1?C.yellow:C.red}}>{pf.toFixed(2)}x</div><div style={{fontSize:10,color:C.textMuted}}>Win/loss ratio</div></div>
              </div>
              <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:4}}>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=><div key={d} style={{textAlign:'center',fontSize:10,color:C.textMuted}}>{d}</div>)}</div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>{Array.from({length:fdm}).map((_,i)=><div key={'e'+i} style={{aspectRatio:'1'}}/>)}{Array.from({length:dim}).map((_,i)=>{const day=i+1,ds=yr+'-'+mos+'-'+String(day).padStart(2,'0'),dd=byDay[ds],dow=(fdm+i)%7,isWe=dow===0||dow===6;return <div key={day} style={{aspectRatio:'1',borderRadius:5,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:isWe?'transparent':dd?(dd.pnl>=0?'rgba(0,170,85,0.15)':'rgba(204,16,64,0.12)'):'#131720',border:'1px solid '+(dd?(dd.pnl>=0?'rgba(0,170,85,0.3)':'rgba(204,16,64,0.25)'):C.border)}}><span style={{fontSize:9,color:C.textMuted}}>{day}</span>{dd&&<span style={{fontSize:9,fontWeight:700,color:dd.pnl>=0?C.synapse:C.red}}>{dd.pnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(dd.pnl)}</span>}</div>})}</div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Win / Loss Breakdown</div><div style={{height:8,borderRadius:4,background:'#1a1f2e',overflow:'hidden',marginBottom:8}}><div style={{height:'100%',width:winPct+'%',background:C.synapse,borderRadius:4}}/></div><div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:14}}><span style={{color:C.synapse,fontWeight:700}}>{winPct}% wins</span><span style={{color:C.red,fontWeight:700}}>{100-winPct}% losses</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Total</span><span style={{fontWeight:700,color:C.text}}>{allT.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Winners</span><span style={{fontWeight:700,color:C.synapse}}>{wins.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Losers</span><span style={{fontWeight:700,color:C.red}}>{losses.length}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Largest win</span><span style={{fontWeight:700,color:C.synapse}}>+{String.fromCharCode(36)}{Math.round(bigW)}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Largest loss</span><span style={{fontWeight:700,color:C.red}}>-{String.fromCharCode(36)}{Math.round(bigL)}</span></div><div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',fontSize:12}}><span style={{color:C.textDim}}>Profit factor</span><span style={{fontWeight:700,color:pf>=1?C.synapse:C.red}}>{pf.toFixed(2)}x</span></div></div>
                <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>Playbook Performance</div>{pbEntries.length===0?<div style={{fontSize:12,color:C.textMuted}}>Log trades with playbooks to see breakdown</div>:pbEntries.map(([name,pb])=>{const pbWr=pb.tot?Math.round(pb.w/pb.tot*100):0;return <div key={name} style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{fontSize:11,color:C.text,fontWeight:600}}>{name}</span><span style={{fontSize:11,fontWeight:700,color:pb.pnl>=0?C.synapse:C.red}}>{pb.pnl>=0?'+':''}{String.fromCharCode(36)}{Math.round(pb.pnl)}</span></div><div style={{display:'flex',gap:8,fontSize:10,color:C.textMuted,marginBottom:4}}><span>{pb.tot} trades</span><span style={{color:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}>{pbWr}% win</span></div><div style={{height:4,borderRadius:2,background:'#1a1f2e',overflow:'hidden'}}><div style={{height:'100%',width:pbWr+'%',background:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}/></div></div>})}</div>
              </div>
              {hrEntries.length>0&&(<div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}><div style={{fontSize:11,fontWeight:700,color:C.textDim,textTransform:'uppercase',letterSpacing:'0.5px',marginBottom:12}}>P&L by Hour of Day</div><div style={{display:'flex',gap:4,alignItems:'flex-end',height:80,marginBottom:8}}>{Array.from({length:14},(_,i)=>i+8).map(h=>{const hp=byHr[h]||0,hpct=Math.abs(hp)/maxHrAbs*100;return <div key={h} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:2}}><div style={{width:'100%',display:'flex',flexDirection:'column',justifyContent:hp>=0?'flex-end':'flex-start',height:60}}><div style={{width:'100%',height:hpct+'%',minHeight:hp!==0?2:0,background:hp>=0?'rgba(0,170,85,0.7)':'rgba(204,16,64,0.7)',borderRadius:2}}/></div><span style={{fontSize:8,color:C.textMuted}}>{h>12?h-12+'p':h+'a'}</span></div>})}</div><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>{topHr&&<div style={{background:'rgba(0,170,85,0.08)',border:'1px solid rgba(0,170,85,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Best hour</div><div style={{fontSize:12,color:C.synapse,fontWeight:700}}>{topHr.h>12?topHr.h-12+'PM':topHr.h+'AM'} (+{String.fromCharCode(36)}{Math.round(topHr.p)})</div></div>}{botHr&&<div style={{background:'rgba(204,16,64,0.06)',border:'1px solid rgba(204,16,64,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Worst hour</div><div style={{fontSize:12,color:C.red,fontWeight:700}}>{botHr.h>12?botHr.h-12+'PM':botHr.h+'AM'} (-{String.fromCharCode(36)}{Math.round(Math.abs(botHr.p))})</div></div>}</div></div>)}
              <div style={{background:'#0d1018',border:'1px solid '+C.tealBorder,borderRadius:10,padding:16}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><div style={{width:8,height:8,borderRadius:'50%',background:C.teal,animation:'pulse 2s infinite'}}/><div style={{fontFamily:fontDisplay,fontSize:12,fontWeight:700,color:C.teal}}>AI Pattern Recognition</div></div><button onClick={analyzePatterns} style={{background:C.tealDim,border:'1px solid '+C.tealBorder,borderRadius:8,padding:'10px 16px',color:C.teal,cursor:'pointer',fontFamily:font,fontSize:12,fontWeight:700,marginBottom:12}}>{aiLoading?'↻ Analyzing...':'🔍 Analyze My Trading Patterns'}</button><div style={{fontSize:11,color:C.textDim,lineHeight:1.6}}>Your trade history, hourly patterns, and playbook data feed the AI companion.</div></div>
            </div>
            )}
          </div>)
        })()}
