        ...
        {tab === 'journal' && (
          <div style={{ flex: 1, overflowY: 'auto', padding: 20, background: '#050609' }}>
            ...
            {trades.length === 0 ? (
              ...
              const allT = trades
              const netPnl = allT.reduce((s: number, t: any) => s + (Number(t.pnl) || 0), 0)
              const wins = allT.filter((t: any) => Number(t.pnl) > 0)
              const losses = allT.filter((t: any) => Number(t.pnl) < 0)
              const winPct = allT.length ? Math.round(wins.length / allT.length * 100) : 0
              const avgW = wins.length ? wins.reduce((s: number, t: any) => s + Number(t.pnl), 0) / wins.length : 0
              const avgL = losses.length ? Math.abs(losses.reduce((s: number, t: any) => s + Number(t.pnl), 0) / losses.length) : 0
              const pf = avgL > 0 ? avgW / avgL : 0
              const bigW = wins.length ? Math.max(...wins.map((t: any) => Number(t.pnl))) : 0
              const bigL = losses.length ? Math.max(...losses.map((t: any) => Math.abs(Number(t.pnl)))) : 0
              const byDay: Record<string, { pnl: number }> = {}
              allT.forEach((t: any) => { const k = String(t.date || ''); if (!byDay[k]) byDay[k] = { pnl: 0 }; byDay[k].pnl += Number(t.pnl) || 0 })
              ...
              allT.forEach((t: any) => { const hv = t.time ? parseInt(t.time) : null; if (hv !== null) { byHr[hv] = (byHr[hv] || 0) + (Number(t.pnl) || 0) } })
              ...
              allT.forEach((t: any) => { const k = String(t.playbook || 'No playbook'); if (!byPb[k]) byPb[k] = { w: 0, tot: 0, pnl: 0 }; byPb[k].tot++; if (Number(t.pnl) > 0) byPb[k].w++; byPb[k].pnl += Number(t.pnl) || 0 })
              ...
              return (
                <div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10, marginBottom: 16 }}>
                    {[
                      { l: 'Net P&L', v: (netPnl >= 0 ? '+' : '') + String.fromCharCode(36) + Math.round(netPnl).toLocaleString(), c: netPnl >= 0 ? C.synapse : C.red, s: allT.length + ' trades' },
                      { l: 'Trade Win %', v: winPct + '%', c: winPct >= 60 ? C.synapse : winPct >= 50 ? C.yellow : C.red, s: wins.length + '/' + allT.length },
                      { l: 'Avg Win', v: '+' + String.fromCharCode(36) + Math.round(avgW), c: C.synapse, s: 'Per winner' },
                      { l: 'Avg Loss', v: '-' + String.fromCharCode(36) + Math.round(avgL), c: C.red, s: 'Per loser' },
                      { l: 'Day Win %', v: dayWpct + '%', c: dayWpct >= 60 ? C.synapse : dayWpct >= 50 ? C.yellow : C.red, s: dayW + '/' + dayArr.length + ' days' },
                      ...
                      <div key={l} style={{ background: '#0d1018', border: '1px solid ' + C.border, borderRadius: 8, padding: '12px 14px' }}>
                        ...
                        <div style={{ fontFamily: fontDisplay, fontSize: 18, fontWeight: 700, color: c, marginBottom: 2 }}>{v}</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4 }}>
                      ...
                      {Array.from({ length: dim }).map((_, i) => {
                        ...
                        return <div key={day} style={{ aspectRatio: '1', borderRadius: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: isWe ? 'transparent' : dd ? (dd.pnl >= 0 ? 'rgba(0,170,85,0.15)' : 'rgba(204,16,64,0.12)') : '#131720', border: '1px solid ' + (dd ? (dd.pnl >= 0 ? 'rgba(0,170,85,0.3)' : 'rgba(204,16,64,0.25)') : C.border) }}>
                          ...
                          {dd && <span style={{ fontSize: 9, fontWeight: 700, color: dd.pnl >= 0 ? C.synapse : C.red }}>{dd.pnl >= 0 ? '+' : ''}{String.fromCharCode(36)}{Math.round(dd.pnl)}</span>}
                        </div>
                      })}
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                    <div style={{ background: '#0d1018', border: '1px solid ' + C.border, borderRadius: 10, padding: 16 }}>
                      ...
                      {[{ l: 'Total', v: allT.length, c: C.text }, { l: 'Winners', v: wins.length, c: C.synapse }, { l: 'Losers', v: losses.length, c: C.red }, { l: 'Largest win', v: '+' + String.fromCharCode(36) + Math.round(bigW), c: C.synapse }, { l: 'Largest loss', v: '-' + String.fromCharCode(36) + Math.round(bigL), c: C.red }, { l: 'Profit factor', v: pf.toFixed(2) + 'x', c: pf >= 1 ? C.synapse : C.red }].map(({ l, v, c }) => (
                        <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid ' + C.border, fontSize: 12 }}>
                          <span style={{ color: C.textDim }}>{l}</span><span style={{ fontWeight: 700, color: c }}>{v}</span>
                        </div>
                      ))}
                    </div>
                    <div style={{ background: '#0d1018', border: '1px solid ' + C.border, borderRadius: 10, padding: 16 }}>
                      ...
                      {pbEntries.length === 0 ? <div style={{ fontSize: 12, color: C.textMuted }}>Log trades with playbooks to see breakdown</div> : pbEntries.map(([name, pb]) => {
                        const pbWr = pb.tot ? Math.round(pb.w / pb.tot * 100) : 0
                        return <div key={name} style={{ marginBottom: 12 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}><span style={{ fontSize: 11, color: C.text, fontWeight: 600 }}>{name}</span><span style={{ fontSize: 11, fontWeight: 700, color: pb.pnl >= 0 ? C.synapse : C.red }}>{pb.pnl >= 0 ? '+' : ''}{String.fromCharCode(36)}{Math.round(pb.pnl)}</span></div>
                  ...
                  <div style={{ background: '#0d1018', border: '1px solid ' + C.tealBorder, borderRadius: 10, padding: 16 }}>
                    ...
                    <button onClick={async () => {
                      setAiLoading(true)
                      try {
                        const pbStr = pbEntries.map(([n, pb]) => n + ': ' + pb.tot + 't ' + Math.round(pb.w / pb.tot * 100) + '% ' + String.fromCharCode(36) + Math.round(pb.pnl)).join(' | ')
                        const hrStr = hrEntries.sort((a, b) => a.h - b.h).map(e => e.h + '=' + String.fromCharCode(36) + Math.round(e.p)).join(', ')
                        const prompt = 'Analyze SPX trader. Win:' + winPct + '% PnL:' + String.fromCharCode(36) + Math.round(netPnl) + ' AvgW:' + String.fromCharCode(36) + Math.round(avgW) + ' AvgL:' + String.fromCharCode(36) + Math.round(avgL) + ' PF:' + pf.toFixed(2) + 'x DayWin:' + dayWpct + '% PB:' + pbStr + ' HR:' + hrStr
                      ...
                      setAiLoading(false)
                    }} style={{ background: C.tealDim, border: '1px solid ' + C.tealBorder, borderRadius: 8, padding: '10px 16px', color: C.teal, cursor: 'pointer', fontFamily: font, fontSize: 12, fontWeight: 700, marginBottom: 12 }}>{aiLoading ? '↻ Analyzing...' : '🔍 Analyze My Trading Patterns'}</button>
