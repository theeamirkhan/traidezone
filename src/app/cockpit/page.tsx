              ...
              {allT.length===0?(
                ...
                <div>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:10,marginBottom:16}}>
                    <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Net P&L</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:netPnl>=0?C.synapse:C.red}}>{netPnl>=0?'+':''}{'$'}{Math.round(netPnl).toLocaleString()}</div><div style={{fontSize:10,color:C.textMuted}}>{allT.length} trades</div></div>
                    ...
                    <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Win</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.synapse}}>+{'$'}{Math.round(avgW)}</div><div style={{fontSize:10,color:C.textMuted}}>Per winner</div></div>
                    <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:8,padding:'12px 14px'}}><div style={{fontSize:10,color:C.textMuted,textTransform:'uppercase',marginBottom:6}}>Avg Loss</div><div style={{fontFamily:fontDisplay,fontSize:18,fontWeight:700,color:C.red}}>-{'$'}{Math.round(avgL)}</div><div style={{fontSize:10,color:C.textMuted}}>Per loser</div></div>
                  ...
                  <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
                      ...
                      {Array.from({length:dim}).map((_,i)=>{const day=i+1,ds=yr+'-'+mos+'-'+String(day).padStart(2,'0'),dd=byDay[ds],dow=(fdm+i)%7,isWe=dow===0||dow===6;return <div key={day} style={{aspectRatio:'1',borderRadius:5,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:isWe?'transparent':dd?(dd.pnl>=0?'rgba(0,170,85,0.15)':'rgba(204,16,64,0.12)'):'#131720',border:'1px solid '+(dd?(dd.pnl>=0?'rgba(0,170,85,0.3)':'rgba(204,16,64,0.25)'):C.border)}}><span style={{fontSize:9,color:C.textMuted}}>{day}</span>{dd&&<span style={{fontSize:9,fontWeight:700,color:dd.pnl>=0?C.synapse:C.red}}>{dd.pnl>=0?'+':''}{'$'}{Math.round(dd.pnl)}</span>}</div>})}
                    </div>
                  </div>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
                    <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}>
                      ...
                      <div style={{display:'flex',justifyContent:'space-between',fontSize:11,marginBottom:14}}><span style={{color:C.synapse,fontWeight:700}}>{winPct}% wins</span><span style={{color:C.red,fontWeight:700}}>{100-winPct}% losses</span></div>
                      <div style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid '+C.border,fontSize:12}}><span style={{color:C.textDim}}>Total</span><span style={{fontWeight:700,color:C.text}}>{allT.length}</span></div>
                    ...
                    <div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16}}>
                      ...
                      {pbEntries.length===0?<div style={{fontSize:12,color:C.textMuted}}>Log trades with playbooks to see breakdown</div>:pbEntries.map(([name,pb])=>{const pbWr=pb.tot?Math.round(pb.w/pb.tot*100):0;return <div key={name} style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}><span style={{fontSize:11,color:C.text,fontWeight:600}}>{name}</span><span style={{fontSize:11,fontWeight:700,color:pb.pnl>=0?C.synapse:C.red}}>{pb.pnl>=0?'+':''}{'$'}{Math.round(pb.pnl)}</span></div><div style={{display:'flex',gap:8,fontSize:10,color:C.textMuted,marginBottom:4}}><span>{pb.tot} trades</span><span style={{color:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}>{pbWr}% win</span></div><div style={{height:4,borderRadius:2,background:'#1a1f2e',overflow:'hidden'}}><div style={{height:'100%',width:pbWr+'%',background:pbWr>=60?C.synapse:pbWr>=50?C.yellow:C.red}}/></div></div>})}
                    </div>
                  </div>
                  {hrEntries.length>0&&(<div style={{background:'#0d1018',border:'1px solid '+C.border,borderRadius:10,padding:16,marginBottom:16}}>
                    ...
                    <div style={{display:'flex',gap:4,alignItems:'flex-end',height:80,marginBottom:8}}>{Array.from({length:14},(_,i)=>i+8).map(h=>{const hp=byHr[h]||0,hpct=Math.abs(hp)/maxHrAbs*100;return <div key={h} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:2}}><div style={{width:'100%',display:'flex',flexDirection:'column',justifyContent:hp>=0?'flex-end':'flex-start',height:60}}><div style={{width:'100%',height:hpct+'%',minHeight:hp!==0?2:0,background:hp>=0?'rgba(0,170,85,0.7)':'rgba(204,16,64,0.7)',borderRadius:2}}/></div><span style={{fontSize:8,color:C.textMuted}}>{h>12?h-12+'p':h+'a'}</span></div>})}</div>
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                      {topHr&&<div style={{background:'rgba(0,170,85,0.08)',border:'1px solid rgba(0,170,85,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Best hour</div><div style={{fontSize:12,color:C.synapse,fontWeight:700}}>{topHr.h>12?topHr.h-12+'PM':topHr.h+'AM'} (+{'$'}{Math.round(topHr.p)})</div></div>}
                      {botHr&&<div style={{background:'rgba(204,16,64,0.06)',border:'1px solid rgba(204,16,64,0.2)',borderRadius:6,padding:'8px 10px'}}><div style={{fontSize:10,color:C.textMuted,marginBottom:2}}>Worst hour</div><div style={{fontSize:12,color:C.red,fontWeight:700}}>{botHr.h>12?botHr.h-12+'PM':botHr.h+'AM'} (-{'$'}{Math.round(Math.abs(botHr.p))})</div></div>}
                    </div>
                  </div>)}
                  <div style={{background:'#0d1018',border:'1px solid '+C.tealBorder,borderRadius:10,padding:16}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}><div style={{width:8,height:8,borderRadius:'50%',background:C.teal,animation:'pulse 2s infinite'}}/><div style={{fontFamily:fontDisplay,fontSize:12,fontWeight:700,color:C.teal}}>AI Pattern Recognition</div></div>
                    <button onClick={analyzePatterns} style={{background:C.tealDim,border:'1px solid '+C.tealBorder,borderRadius:8,padding:'10px 16px',color:C.teal,cursor:'pointer',fontFamily:font,fontSize:12,fontWeight:700,marginBottom:12}}>{aiLoading?'↻ Analyzing...':'🔍 Analyze My Trading Patterns'}</button>
                    <div style={{fontSize:11,color:C.textDim,lineHeight:1.6}}>Your trade history, hourly patterns, and playbook data feed the AI companion.</div>
                  </div>
                </div>
              )}
            </div>
